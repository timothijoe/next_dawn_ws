...
#include <ctime>
#include <chrono>
#include "../task_manager/flowsegments.h"
...
struct Particle{
  int id;
  int time_horizon;
  vector<double> gap_size_vec;
  vector<Vector2d> gap_location_vec;
...
{
public:
  pointSampler(ros::NodeHandle nh);
  metaTask _task_type = INTO_FLOW;
...
//  int current_seg_id = -1;
  ...
  bool generate_trajectories(const double velocity, \
  ...
  bool reset_target_pos_func(pedsim_srvs::ResetRobotPos::Request &req,
...
//  bool updateCurId();
  ...
  //here we will formally use particle filter
  int num_particles;
  bool is_initialized;
...
//  void updateWeights(double sensor_range, double std_landmark[], vector<LandmarkObs> osbservations, Map map_landmarks);
//  void resample();
  ...
  ros::ServiceServer _resetTargetPosServer;
...
Vector3d pointSampler::localToOdom(Vector3d local_goal)
{
  Vector3d odom_goal;
...
Vector3d pointSampler::odomToLocal(Vector3d goal)
{
  Vector3d local_goal;
