...
void MotionModelDiffDrive::generate_energy(const double dt, const double trajectory_length, const VelocityParams& _vel, const double k0, const double km, const double kf, Eigen::Vector3d& output,double& energy)
{
  AngularVelocityParams omega(k0, km, kf, trajectory_length);
  VelocityParams vel = _vel;
  vel.time = estimate_driving_time(ControlParams(vel, omega));
...
bool MotionModelDiffDrive::get_hamiltonian_from_param(const double dt, const double trajectory_length, const VelocityParams& _vel, const double k0,
                                                      const double km, const double kf, double& hamilton_, Eigen::Vector3d& final_pos_)
{
  hamilton_ = 0;
  double vel_part = 0;
  double density_part = 0;
  ...
  double sf_2 = omega.sf * ratio;
  State state(0, 0, 0, vel.v0, omega.k0);
  final_pos_ << state.x, state.y, state.yaw;
  for(int i=1;i<N;i++){
      double s = s_profile[i];
      double k = 0;
      if(s < sf_2){
      ...
      update(state, v_profile[i], k, dt, state);
     // get hamilton from the state
      //decision_maker->odomToLocal(1,1,0);
      //decision_maker->isOccluded(state.x, state.y);
      double density_ = 0;//= decision_maker->getDensity(state.x, state.y);

      //double density_ = taskManger->getDensity(state.x, state.y);
      //decision_maker->getDensity(state.x, state.y);
      //std::cout <<"decision maker id is : :" <<decision_maker->dec_maker_id << std::endl;
      //std::cout << "density is ge ming bi sheng "<< density_ << std::endl;
      Vector2d vel_map_(1,0) ;// = decision_maker->getVelocity(state.x, state.y);
      //Vector2d vel_map = taskManger->getVelocity(state.x, state.y);
      Vector2d vel_traj_(-cos(state.yaw), -sin(state.yaw));
      double vel_cost_ = vel_map_[0] * vel_traj_[0] + vel_map_[1]+vel_traj_[1];
      double vel_cost = vel_cost_;
      //double vel_cost = vel_cost_ > 0? 0.1 * vel_cost_ : 0;
      vel_part = vel_part + vel_cost;
      density_part = density_part+ density_;
      double one_step_cost = density_ + vel_cost;
      hamilton_ += one_step_cost;
  }
  std::cout << "density part: " << density_part << std::endl;
  std::cout << "velocity part: " << vel_part << std::endl;
  std::cout << "traj_len : " << N << std::endl;
  final_pos_ << state.x, state.y, state.yaw;
}
