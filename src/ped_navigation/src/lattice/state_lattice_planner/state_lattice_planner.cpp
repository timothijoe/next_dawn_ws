...
void StateLatticePlanner::load_lookup_table(const std::string& LOOKUP_TABLE_FILE_NAME)
{
    ...
    std::cout<<"Suceess load lookup table!" << std::endl;
}
...
void StateLatticePlanner::generate_biased_point_states(const int n_s, const Eigen::Vector3d& goal, double target_velocity, std::vector<Eigen::Vector3d>& states)
{
  //double alpha_coeff = 0.40;
  double goal_distance = goal.segment(0,2).norm();
  double alpha_coeff = 0.40 / goal_distance;
  double angle_base = atan2(goal[1],goal[0]);
  double angle_left = angle_base - alpha_coeff;
  double angle_right = angle_base + alpha_coeff;
  for(double angle = angle_left; angle <= angle_right; ){
    for(double dist = goal_distance - 0.2; dist <= goal_distance + 0.2;){
      Eigen::Vector3d terminal;
      terminal << dist * cos(angle),
                  dist * sin(angle),
                  goal[2];
...
//        auto z_start = std::chrono::system_clock::now();
        ...
        //param.omega.sf = boundary_state.segment(0, 2).norm();
        // std::cout << "v0: " << velocity << ", " << "k0: " << k0 << ", " << "km: " << param.omega.km << ", " << "kf: " << param.omega.kf << ", " << "sf: " << param.omega.sf << std::endl;
//        auto z_end = std::chrono::system_clock::now();
//        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(z_end - z_start);
//        std::cout<<"it takes"
//           << double(duration.count()) * std::chrono::microseconds::period::num / std::chrono::microseconds::period::den
//           << "seconds to get lookup table" << std::endl;
