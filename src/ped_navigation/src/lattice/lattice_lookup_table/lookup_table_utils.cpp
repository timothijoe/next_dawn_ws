...
namespace LookupTableUtils
{
    ...
    void get_optimized_params_from_lookup_table(const LookupTable& lookup_table, const Eigen::Vector3d goal, const double v0, const double k0, vector<MotionModelDiffDrive::ControlParams> &params_)
    {
      MotionModelDiffDrive::ControlParams c_param;
      params_.clear();
      if(lookup_table.size() > 0){
          double min_v_diff = 1e3;
          double v = 0;
          ...
          for(const auto& data : lookup_table.at(v).at(k)){
              // sqrt(x^2 + y^2 + yaw^2)
              double cost = (goal - data.state).segment(0,2).norm();
              //double angle_cost = abs(goal[2] - data.state[2]);
              double angle_cost = (goal[2] - data.state[2]);
              //if(cost < min_cost && (angle_cost < 0.1 || angle_cost>0)){
              if(cost < min_cost && (angle_cost < 0.9 && angle_cost> 0)){
                std::cout <<"new world: " <<data.state << std::endl;
                  _param = data;
                  params_.push_back(_param.control);
              }
          }
          //param = _param.control;
      }else{
          c_param.omega.km = 0;
          c_param.omega.kf = 0;
          c_param.omega.sf = goal.segment(0, 2).norm();
          params_.push_back(c_param);
      }
  }
