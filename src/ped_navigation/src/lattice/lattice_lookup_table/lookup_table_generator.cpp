#include "lattice/lattice_lookup_table/lookup_table_generator.h"
...
:local_nh("~")
{
    ...
    local_nh.param("LOOKUP_STATE_TABLE_NAME", LOOKUP_STATE_TABLE_FILE_NAME, {std::string(std::getenv("HOME")) + "/lookup_state_table.csv"});
    ...
    local_nh.param("MAX_YAWRATE", MAX_YAWRATE, {0.4}); //0.8
    local_nh.param("MAX_D_YAWRATE", MAX_D_YAWRATE, {1.0}); //2.0
        ...
        for(double k0=-MAX_KAPPA;k0<=MAX_KAPPA;k0+=DELTA_KAPPA){
            for(auto state : states){
                ...
                if(cost > 0){
                    ...
                    data << v0 << "," << k0 << "," << round(trajectory.trajectory.back()(0)*10000)/10000 << "," << round(trajectory.trajectory.back()(1)*10000)/10000 << "," << round(trajectory.trajectory.back()(2)*10000)/10000 << "," << round(output.omega.km*10000)/10000 << "," << round(output.omega.kf*10000)/10000 << "," << round(output.omega.sf*10000)/10000 << "\n";
...
std::string LookupTableGenerator::state_process()
{
  const int N_X = (MAX_X - MIN_X) / DELTA_X + 1;
  if(N_X < 1){
      std::cout << "param error(x)" << std::endl;
...
//  }
  ...
  //std::vector<std::vector<double> > lookup_table_data_list;
  //lookup_table_data_list.resize(N);
  std::vector<double> lookup_table_data;
  lookup_table_data.resize(6);
  int success_count = 0;
  std::string output_data = "x0, y0, yaw0, x, y, yaw, km, kf, sf\n";
  // Notice, the initial guess is estimated when v0 = 1m/s and omega = 0.
  // Resolution is 0.1 m for x, y and pi / 6 for yaw.
  for(auto state : states){
    ...
    // Initialize the optimized param
    optimized_param.omega.sf = distance;
    // parameter v0, a0, vt, vf, af
    MotionModelDiffDrive::VelocityParams init_v(0.6, 1.0, 0.6, 0.6, 1.0);
    // k0, km, kf, sf
    MotionModelDiffDrive::AngularVelocityParams init_omega(0.0, optimized_param.omega.km, optimized_param.omega.kf, optimized_param.omega.sf);
    MotionModelDiffDrive::ControlParams init(init_v, init_omega);
    MotionModelDiffDrive::ControlParams output;
    MotionModelDiffDrive::Trajectory trajectory;
    TrajectoryGeneratorDiffDrive tg;
    tg.set_motion_param(MAX_YAWRATE, MAX_D_YAWRATE, MAX_ACCELERATION, MAX_WHEEL_ANGULAR_VELOCITY, WHEEL_RADIUS, TREAD);
    ...
    if(cost > 0){
        std::cout << "\033[032msuccessfully optimized\033[0m" << std::endl;
        std::stringstream data;
        data << state[0] << ","<<state[1] << "," << state[2] << "," << round(trajectory.trajectory.back()(0)*10000)/10000 << "," << round(trajectory.trajectory.back()(1)*10000)/10000 << "," << round(trajectory.trajectory.back()(2)*10000)/10000 << "," << round(output.omega.km*10000)/10000 << "," << round(output.omega.kf*10000)/10000 << "," << round(output.omega.sf*10000)/10000 << "\n";
        output_data += data.str();
        success_count++;
    }else{
        std::cout << "\033[031mfailed to optimize trajectory\033[0m" << std::endl;
...
void LookupTableGenerator::state_save(std::string &data)
{
  std::ofstream ofs(LOOKUP_STATE_TABLE_FILE_NAME);
  if(ofs){
      ofs << data;
      ofs.close();
      std::cout << "lookup table saved as " << LOOKUP_STATE_TABLE_FILE_NAME << std::endl;
      exit(0);
  }else{
      std::cout << "cannot open file" << std::endl;
      exit(-1);
